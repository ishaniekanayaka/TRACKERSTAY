...
import {
  StyleSheet, Text, View, TouchableOpacity, ScrollView,
  ActivityIndicator, Alert, RefreshControl, Modal,
  Animated, Image, Linking, NativeSyntheticEvent, NativeScrollEvent,
} from 'react-native';
import { Ionicons } from '@expo/vector-icons';
import DateTimePicker from '@react-native-community/datetimepicker';
import HeaderWithMenu from '../../components/HeaderWithMenu';
...
const BookingCalendar = () => {
  ...
  // We keep one ref array. Each category registers its ScrollView here.
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const res = await bookingCalendarService.getCalendarBookings(monthFirstDay);
      setData(res);
      // Reset scroll refs array size when data changes
      scrollRefs.current = [];
    } catch (err) {
      Alert.alert('Error', err instanceof Error ? err.message : 'Failed to load');
    } finally { setLoading(false); setRefreshing(false); }
  ...
  // When month changes, reset scroll position
  useEffect(() => {
  ...
  const handleScroll = useCallback((e: NativeSyntheticEvent<NativeScrollEvent>, sourceIndex: number) => {
    if (isSyncingScroll.current) return;
    const x = e.nativeEvent.contentOffset.x;
    currentScrollX.current = x;
    isSyncingScroll.current = true;
    scrollRefs.current.forEach((ref, idx) => {
      if (idx !== sourceIndex && ref) {
  ...
  // ── Render category ───────────────────────────────────────────────────────
  // catIndex is used to register/retrieve the correct ScrollView ref
  const renderCategory = (cat: CalendarCategory, catIndex: number) => (
    <View key={`cat-${cat.category_id}`} style={s.catCard}>
      <View style={s.catBar}>
        <Ionicons name="bed" size={16} color="#fff"/>
        <Text style={s.catBarTxt}>{cat.category_name}</Text>
      ...
      <View style={s.tableWrap}>
        ...
        <ScrollView
          horizontal
          showsHorizontalScrollIndicator={catIndex === (data?.categories.length ?? 1) - 1}
          persistentScrollbar={catIndex === (data?.categories.length ?? 1) - 1}
          style={s.hScroll}
          scrollEventThrottle={16}
          ref={(ref) => { scrollRefs.current[catIndex] = ref; }}
          onScroll={(e) => handleScroll(e, catIndex)}
          contentOffset={{ x: currentScrollX.current, y: 0 }}
        >
          <View style={{width:totalDayW}}>
            <View style={s.dayHeadRow}>
              {dayList.map((dateStr,i) => {
    ...
    <View style={s.container}>
      ...
      {loading&&!refreshing ? (
        ...
        <ScrollView style={s.scroll}
          ...
          ) : data.categories.map((cat, catIndex) => renderCategory(cat, catIndex))}
          <View style={{height:80}}/>
        </ScrollView>
      )}
