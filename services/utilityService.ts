...
//   color?: string;
//   [key: string]: any; // For dynamic category values like Water, Electricity, etc.
// }
...
// export interface NewChartDataResponse {
//   chartData: CategoryChartData[];
//   total_count: TotalCount[];
//   total_bill_count: TotalBillCount[];
// }
...
//   /**
//    * Get chart data from NEW API endpoint (GET method)
//    * GET /api/utility/dashboard/chart_render_utility?selected_date_utility=YYYY-MM-DD
//    */
//   async getChartData(selectedDate: string): Promise<NewChartDataResponse> {
//     const response = await api.get<NewChartDataResponse>(
//       '/utility/dashboard/chart_render_utility',
//       { params: { selected_date_utility: selectedDate } }
//     );
...
//   category: string;
//   category_id: number;
//   total_bill: string;  // This is "Charge"
// }
...
//   category: string;
//   category_id: number;
//   total_BillReading: string;  // This is "Bill charges"
// }

// // New API Response Structure
// export interface NewChartDataResponse {
//   chartData: CategoryChartData[];
//   total_count: TotalCount[];      // Contains "Charge" (total_bill)
//   total_bill_count: TotalBillCount[];  // Contains "Bill charges" (total_BillReading)
// }
...
class UtilityService {
  ...
  async getUtilities(): Promise<UtilityItem[]> {
    try {
      ...
      throw new Error(error.response?.data?.message || 'Failed to fetch utilities');
    }
  }
  ...
  async getUtilityById(id: number): Promise<UtilityItem> {
    try {
      ...
      throw new Error(error.response?.data?.message || 'Failed to fetch utility');
    }
  }
  ...
  async saveUtility(data: FormData): Promise<UtilityItem> {
    try {
      ...
      throw new Error(error.response?.data?.message || 'Failed to save utility');
    }
  }

  async updateUtility(id: number, data: FormData): Promise<UtilityItem> {
    try {
      const response = await api.post(`/utility/update/${id}`, data, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      return response.data.data;
    } catch (error: any) {
      console.error('Error updating utility:', error);
      throw new Error(error.response?.data?.message || 'Failed to update utility');
    }
  }
  ...
  async deleteUtility(id: number): Promise<void> {
    try {
      ...
      throw new Error(error.response?.data?.message || 'Failed to delete utility');
    }
  }
}
